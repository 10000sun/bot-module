...
class MariState:
    ...
    `from mari_state import user_ids`로 **자기 모듈에 별도 이름을 묶어두기** 때문에,
    누군가 `mari_state.user_ids = 새딕셔너리`로 재할당하면 이미 import된 코그들은
    ...
    def load(self):
        ...
        # 🗑️ [정리] mari_access(레거시 개별 허용 역할 목록)는 삭제했습니다.
        # 이 목록을 추가/삭제하는 명령어가 코드 어디에도 없어 사실상 죽은 기능이었고,
        # 지금은 `/설정 관리자 아이디`로 지정하는 ids_admin 역할이 같은 역할을 하고 있습니다.
        legacy = self._raw.get("mari_access")
        if legacy:
            print(f"ℹ️ [안내] ids.json에 남아있던 레거시 mari_access 데이터는 더 이상 사용되지 않습니다: {legacy}")
    ...
    def save(self):
        ...
        #  • mari_access    : 위 load() 설명 참고
        #  • log_channels   : 로그 채널 지정은 settings.json의 channels.*로 완전히 이관됐어요
        #  • current_numbers: 하이로우 게임의 임시 숫자. MariGames가 메모리에서만 들고 있어요
        for dead_key in ("mari_access", "log_channels", "current_numbers"):
