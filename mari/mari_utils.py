...
async def report_broken_transaction(
    ...
    저장 순서는 이미 **가치가 사라지는 쪽**(유저가 손해 보는 쪽)으로 잡혀 있어서 에바가
...
class MariView(discord.ui.View):
    """마리봇의 모든 버튼/드롭다운 창이 상속하는 기본 View.
...
# (오타거나 진짜 새로운 플랫폼이거나) 마리가 알아서 등록하지 않고 관리자에게 확인을 받습니다.
