...
from mari_config import module_active
...
class MariHelp(commands.Cog):
    ...
    _FEATURE_OWNERS = ("id", "economy", "stock", "wiki", "shop", "birthday", "snooze")
    ...
    def _admin_categories(self):
        ...
        return self._visible({
            "서버/시스템 관리": ("🛡️", [
                ...
                ("economy", f"`/통계` - 출석률·{currency()} 유통량·거래 활발도·주식 현황 요약"),
                (None, "`/감사로그` - 담긴 기능의 로그를 통합 조회 (경제·상점·아이디·역할·주식·생일 중)"),
            ...
            "경제/상점 관리": ("💰", [
                ...
                ("economy", "`/지갑 전체:True` - 서버 인원 전원의 지갑·주식·합계를 자산 많은 순으로 조회 (예전 랭킹 명령을 대체해요)"),
            ]),
            "주식 특수 관리": ("📈", [
                ("stock", "`/주식 생성`, `/주식 삭제`"),
            ...
            "테스트/진단 도구": ("🧪", [
                ...
                (None, "`/테스트 데이터점검` - 아이디 중복·상점 설정 누락·파일 손상·저장 실패 일괄 점검"),
    ...
    def _user_categories(self):
        ...
        return self._visible({
            "지갑 및 상점": ("💰", [
                ...
                ("economy", f"`/지갑내역 조회` - 내 {currency()}{josa(currency(), '이가')} 언제 얼마나 들어오고 나갔는지 확인 (송금·출석·상점·주식 전부)"),
