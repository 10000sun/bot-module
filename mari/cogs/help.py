...
class MariHelp(commands.Cog):
    ...
    def _admin_categories(self):
        ...
        return {
            "서버/시스템 관리": ("🛡️", (
                "`/초기설정` - 재화·봇·이벤트·서버 이름 지정 (봇을 처음 들였을 때 제일 먼저. 나중에 다시 바꿔도 돼요)\n"
                "`/설정 관리자` - 기능별 관리자 역할 지정\n"
                "`/설정 채널` - 시스템/로그 전용 채널 맵핑\n"
                "`/설정 채널지정내역` - 지금 지정돼 있는 채널·관리자 역할을 한눈에 확인\n"
                f"`/통계` - 출석률·{currency()} 유통량·거래 활발도·주식 현황 요약\n"
            ...
            "게임 아이디 관리": ("🎮", (
                ...
                f"`/설정 채널 아이디등록` - 이 채널에 유저가 올린 '플랫폼 아이디'를 {bot_name()}{josa(bot_name(), '이가')} 자동으로 등록/삭제 처리\n"
                "└ 맨 앞에 `변경` 또는 `수정`을 붙이면(예: `변경 라이엇 새아이디#kr1`) 무조건 관리자 승인을 거쳐서 같은 플랫폼의 기존 값을 바꿔줘요\n"
                "`/설정 채널 아이디목록` - 아이디 명단이 자동 갱신될 채널 지정 (👑대장 / 멤버 두 칸으로 올라가요)\n"
                "└ 이 채널에 아이디 관리자가 \"갱신\"이라고 치면 그 자리에서 바로 새로고침돼요\n"
                "`/설정 명단 대장` - 명단에서 '대장' 칸으로 따로 표시할 역할 지정\n"
                "`/아이디 가져오기` - 예전에 쓰던 아이디 목록 게시글(.txt)을 한 번에 등록\n"
            ...
            "경제/상점 관리": ("💰", (
                ...
                f"`/지급` & `/회수` - {currency()} 일괄 통제\n"
                "`/지급취소` - 최근 지급/회수를 목록에서 골라 되돌리기 (잘못 지급했을 때 복구용)\n"
                f"`/지갑내역 유저조회` - 특정 유저의 {currency()} 입출금 내역 확인 (문의 대응용)\n"
                "`/출석보상설정` - 기본 보상 금액 조정\n"
                "`/지갑청소` - 퇴장 유저 지갑 정리 (지우기 전에 대상 목록을 먼저 보여주고 확인 버튼을 눌러야 실행돼요)\n"
                "`/지갑 전체:True` - 서버 인원 전원의 지갑·주식·합계를 자산 많은 순으로 조회 (예전 랭킹 명령을 대체해요)"
            )),
            "주식 특수 관리": ("📈", (
                "`/주식 생성`, `/주식 삭제`\n"
                "`/주식 변동` - 주가 변동 및 사유 예약\n"
            ...
            "정보 및 역할 관리": ("👥", (
                "`/역할부여` - 특정 멤버 역할 지급 (최대 25개까지 선택 UI로. 서버 관리자 전용)"
            )),
            ...
            "이벤트 관리": ("🎉", f"`/이벤트설정` - {event_name()} 이벤트 선착순 인원/금액/지속시간 조정 (파라미터 없이 실행 시 현재 설정 확인)"),
            "테스트/진단 도구": ("🧪", (
                ...
                f"`/테스트 이벤트` - {event_name()} 이벤트 창을 짧게 강제로 열어서 전체 흐름 테스트\n"
                "`/테스트 출석초기화` - 본인 오늘 출석 기록만 지워서 반복 테스트\n"
                "`/테스트 아이디파싱` - 실제 등록 없이 텍스트 파싱 결과만 미리보기\n"
                "`/테스트 ai상태` - Gemini RPM/RPD 사용량 및 API 연결 확인\n"
    ...
    def _user_categories(self):
        ...
        return {
            "지갑 및 상점": ("💰", (
                "`/지갑` - 잔고 및 소지품 확인\n"
                f"└ 🎁 **선물하기** 버튼으로 상점에서 산 아이템을 다른 사람에게 넘겨줄 수 있어요 ({currency()}{josa(currency(), '은는')} 안 나가요)\n"
                "└ 역할이 지급되는 아이템은 선물이 안 돼요. 되팔기로 정리한 뒤 상대가 직접 사면 됩니다\n"
                f"`/지갑내역 조회` - 내 {currency()}{josa(currency(), '이가')} 언제 얼마나 들어오고 나갔는지 확인 (송금·출석·상점·주식 전부)\n"
                f"`/송금` - 다른 유저에게 {currency()} 이체\n"
                "`/출석` - 매일 출석체크 및 보상 획득\n"
                "상점 매대는 채널마다 따로 있어요! 되팔기 환급 비율은 상품마다 달라요.\n"
                "🖱️ 멤버를 **우클릭 → 앱**하면 `송금하기`·`아이템 선물하기`가 바로 열려요. "
            ...
            "생일": ("🎂", (
                ...
                "`/생일 삭제` - 등록한 생일 정보 삭제\n"
                "`/생일 확인` - 특정 멤버의 생일 확인\n"
                f"생일 당일이 되면 {bot_name()}{josa(bot_name(), '이가')} 축하 채널에 자동으로 알려줘요!"
            )),
            "미니게임 및 아이템": ("🎲", (
                "`/하이로우`, `/하이`, `/로우` - 간단한 예측 게임\n"
                f"매일 00:21, 12:21에 아무 채널에서나 \"{event_name()}\"라고 치면 {currency()}{josa(currency(), '을를')} 받을 수 있어요! (참가자 전원 결과는 이벤트 종료 후 안내 채널에 한 번에 올라와요)"
            )),
            "나중에 답장": ("⏰", (
            ...
            f"AI {bot_name()} 대화": ("💬", (
                f"{bot_name()}봇 호출 구문이나 전용 역할을 멘션해서 말을 걸어보세요! 이미지를 같이 올리면 {bot_name()}{josa(bot_name(), '이가')} 사진도 보고 대답해줘요.\n"
                f"{bot_name()}{josa(bot_name(), '이가')} 대화 중 기억해둘 만한 걸 알아서 기억해요. `/기억 목록`으로 확인, `/기억 삭제`·`/기억 초기화`로 지울 수 있어요.\n"
