...
class MariHelp(commands.Cog):
    ...
    ADMIN_CATEGORIES = {
        ...
        "게임 아이디 관리": ("🎮", (
            ...
            "`/아이디 공지` - 레벨 명단 맨 아래에 공지 한 줄 추가 (기본은 누적, 덮어쓰기 옵션 있음. 아이디 관리자 전용)\n"
        ...
        "경제/상점 관리": ("💰", (
            ...
            "`/지갑 캠프:전체` - 서버 인원 전원의 지갑·주식·합계를 자산 많은 순으로 조회 (예전 `/랭킹`을 대체해요)"
        )),
        "주식 특수 관리": ("📈", (
            "`/주식 생성`, `/주식 삭제`\n"
            "`/주식 변동` - 주가 변동 및 사유 예약\n"
        ...
        "정보 및 역할 관리": ("👥", (
            ...
            "`/프로필설정 역할등록`·`역할해제` - 레벨·소속·직책 카테고리에 역할 등록/해제\n"
            "`/프로필설정 업적추가` - 유저에게 업적 부여 (만들어둔 업적은 목록에서 골라요. 새로 입력하면 목록에 자동 등록)\n"
            "`/프로필설정 업적삭제` - 특정 유저에게서만 업적 회수 (목록은 그대로)\n"
            "`/프로필설정 업적목록` - 만들어둔 업적과 각각 보유 인원 확인\n"
            "`/프로필설정 업적수정` - 업적 문구 변경 (갖고 있는 사람 전원에게 한 번에 반영)\n"
            "`/프로필설정 업적목록삭제` - 업적을 목록에서 없애고 보유자 전원에게서 회수\n"
            "`/프로필설정 사진`·`사진삭제` - 프로필 사진 등록/삭제"
        )),
        ...
        "캠프장 전용": ("🏕️", (
            "**🧾 세금 — 캠프마다 규칙이 달라요**\n"
            "우리 캠프 기준은 마리가 알아서 적용하니, 세율을 매번 계산해서 넣지 않으셔도 돼요.\n"
            "> 🕊️ **여백** — 지갑 초과 누진세 **+ 주식 보유세(평가액의 1%)** + 로또 티켓 자동 계산\n"
            "> 🌈 **나래** — 구간별 단일세율 (지갑 **+ 주식 평가액** 합산)\n"
            "> 😈 **악동** — 세금 룰렛 (참여자는 뽑기 / 미참여자는 최고액 고정)\n\n"
            "**1️⃣ 먼저 얼마나 걷힐지 봅니다**\n"
            "`/캠프 세금환수 캠프:우리캠프`\n"
            "└ 그냥 실행하면 **미리보기**예요. 실수로 걷힐 일이 없으니 편하게 눌러보세요.\n"
            "└ 누가 얼마 내는지, 지갑이 모자라 못 내는 사람은 누군지 명단으로 나와요.\n\n"
            "**2️⃣ 확인했으면 진짜로 걷습니다**\n"
            "`/캠프 세금환수 캠프:우리캠프 실제징수:True`\n"
            "└ 걷은 세금은 캠프 통장에 자동으로 쌓여요.\n"
            "└ 😈 악동은 `참여자역할:@역할`을 같이 넣어주세요. 그 역할이 있는 사람만 룰렛을 돌리고, "
            "없는 사람은 미참여로 최고액을 냅니다. (안 넣으면 전원 미참여 처리)\n"
            "└ 🌈 나래 첫 회처럼 **전원 똑같은 비율**로 걷고 싶으면 `정률:10`을 넣으세요. 그때만 캠프 기준을 무시해요.\n\n"
            "**⚠️ 미납이 뜬다면**\n"
            "세금이 그 사람 지갑보다 크면 있는 만큼만 걷고 나머지는 `⚠️미납`으로 표시돼요. "
            "이 명단으로 악동은 노비 처리를, 나래는 미납자 강제 징수를 하시면 됩니다.\n\n"
            "**⚙️ 세율·확률을 바꾸고 싶을 때**\n"
            "`/캠프 세금기준` - 우리 캠프 기준이 지금 어떻게 되어 있는지 확인\n"
            "`/캠프 세금기준수정` - 창이 열리면 **숫자만** 고쳐서 제출 (형식은 그대로 두세요)\n"
            "└ 😈 악동은 `룰렛 = 30000:1, 20000:3, ...`에서 뒤 숫자가 확률 비중이에요. 많이 걸린다 싶으면 여기만 조정하세요.\n"
            "└ 🕊️ 여백은 `주식보유세 = 1`이 주식 세율(%)이에요. `0.5`로 낮추거나 `0`으로 끄실 수 있어요.\n"
            "└ 잘못 적으면 저장 안 하고 어디가 틀렸는지 알려주니 겁내지 않으셔도 돼요.\n"
            "└ 바꾼 기준은 다음 세금환수부터 바로 적용돼요. (봇 재시작 불필요)\n\n"
            "**🏦 캠프 통장**\n"
            "`/캠프 조회` - 캠프별 통장 잔액 확인\n"
            "`/캠프 인출` - 통장에서 내 지갑으로 인출\n"
            "`/캠프 송금` - 통장에서 특정 유저에게 지급 (컨텐츠 기여자 보상 등)\n"
            "`/캠프 회수` - 특정 유저 지갑에서 통장으로 회수\n\n"
            "**🚌 견학**\n"
            "`/견학 보내기` - 멤버를 다른 캠프로 견학 (역할 자동 교체, 세금 대상에서 자동 제외)\n"
            "└ 🎫 `일일견학권:True`로 보내면 대상자 인벤토리에서 `타 캠프 1일 견학권`을 차감해요. 없으면 안 보내져요\n"
            "└ 📅 **시간이 아니라 날짜 기준이에요.** 1장 = 오늘 자정까지, 2장 = 내일 자정까지\n"
            "└ 밤 11시 59분에 쓰면 1분 뒤 자정에 끝나요. `일수`에 산 장수만큼 넣으세요\n"
            "└ 이미 견학 중인 사람에게 또 태우면 **마지막 날 뒤에 이어붙어요** (1장씩 3번 = 3일권)\n"
            "└ 상점주인이 보내면 일일견학권으로 자동 기록돼요\n"
            "`/견학 조회` - 현재 견학 중인 인원 확인 (일일견학권은 만료일·남은 일수까지 표시)\n"
            "`/견학 복귀` - 특정 캠프 원 소속자(또는 전원) 일괄 복귀\n"
            "└ ⚠️ 자정이 지나도 자동 복귀되지 않아요. 조회에 만료 표시가 뜨면 직접 복귀시켜 주세요\n"
            "`/견학 내역` - 전체 견학 이동 기록 조회\n"
            "└ `보내기`·`복귀`·`조회`는 **상점주인도** 쓸 수 있어요. `내역`만 캠프장 전용이에요\n\n"
            "**🎫 견학권을 사면 마리가 먼저 물어봐요**\n"
            "구매자에게 DM이 가서 희망 캠프(버튼)와 날짜(직접 입력)를 받아, **상점 로그 채널**로 "
            "상점주인을 태그해서 올려줍니다. 그걸 보고 `/견학 보내기`로 태워주시면 돼요.\n"
            "└ 구매자가 DM을 막아뒀거나 3일 안에 답을 안 하면 상점 로그에 그 사실이 올라와요\n"
            "└ 상점 로그 채널이 지정돼 있어야 전달돼요 (`/설정 채널 상점로그`)"
        )),
        "테스트/진단 도구": ("🧪", (
    ...
    USER_CATEGORIES = {
        "지갑 및 상점": ("💰", (
            ...
            "🎫 **견학권**을 사면 마리가 DM으로 어느 캠프에 언제 가고 싶은지 물어봐요. "
            "버튼으로 캠프를 고르고 날짜만 적으면 상점주인에게 바로 전달돼요. (3일 안에 답해주세요)\n"
        ...
        "프로필": ("👤", (
            "`/프로필 [유저]` - 닉네임·아이디·레벨·소속·직책·업적·사진을 예쁜 카드로 확인 (유저 생략 시 본인)\n"
            "🖱️ 멤버 **우클릭 → 앱 → 프로필 보기**로도 바로 열려요 (나만 보여요)"
        )),
        "정보 조회": ("📖", (
    ...
    async def help_command(self, interaction: discord.Interaction, 관리자: bool = False):
        ...
        if not is_admin:
            try:
                ...
                # 서버 관리자와 타운가이드 말고는 아무도 관리자 도움말을 못 여는 상태가 돼요.
                ...
                # 유저가 가진 역할 중 하나라도 관리자/가이드 계열에 속하면 통과
