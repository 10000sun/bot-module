"""MariSelfRole — 버튼으로 스스로 붙였다 떼는 역할(셀프 역할) 패널.
...
🔐 **권한 상승을 막는 게 이 기능의 전부입니다.** 셀프 역할은 "아무나 눌러서 가져가는
   ...
   두 검사는 입장 자동 역할과 같은 것이라 mari_utils에 함께 뒀어요.
...
class SelfRolePanelView(MariView):
    """패널 하나에 붙는 버튼 묶음. timeout=None이라 봇을 재시작해도 계속 눌려요."""

    def __init__(self, cog: "MariSelfRole", panel_id: str, roles: list):
...
class MariSelfRole(commands.Cog):
