...
class ImportConfirmView(MariView):
    ...
    def __init__(self, core_cog, guild: discord.Guild, matched: list, unmatched: list = None):
        ...
        self.guild = guild
        self.matched = matched      # (level, name, discord.Member, {platform: id})
        self.unmatched = unmatched or []  # (level, name, {platform: id})
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for level, name, member, ids in self.matched:
            uid = str(member.id)
        ...
        state.save()
        await self.core_cog._refresh_level_roster(self.guild)
...
class MariCore(commands.Cog):
    ...
    async def set_id_roster_notice(self, interaction: discord.Interaction, 내용: Optional[str] = None, 덮어쓰기: bool = False, 삭제: bool = False):
        ...
        # ⚠️ asyncio.Lock은 재진입이 안 되니까, _refresh_level_roster 호출은 반드시 이 블록 밖에서 해요.
        need_refresh = False
        async with self.bot.settings_lock:
            settings = load_settings()
        ...
        if need_refresh:
            await self._refresh_level_roster(interaction.guild)
    ...
    async def reload_id_data(self, interaction: discord.Interaction):
        ...
        await self._refresh_level_roster(interaction.guild, force_repost=True)
    ...
    async def _resolve_id_pending_from_reply(self, message: discord.Message):
        ...
        await self._refresh_level_roster(guild)
    ...
    async def on_message(self, message: discord.Message):
        ...
        if roster_ch_id and message.channel.id == roster_ch_id and message.content.strip() == "갱신":
            ...
            try:
                ...
                pass
            await self._refresh_level_roster(message.guild, force_repost=True)
    ...
    async def _process_id_submission(self, message: discord.Message):
        ...
        if registered:
            state.save()
            await self._refresh_level_roster(message.guild)
