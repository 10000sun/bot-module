...
예전엔 "에바"·"마리"·"에바시"·"에바스타운"이 코드 곳곳의 문자열에 그대로 박혀 있었어요.
...
⚠️ 조사를 손으로 붙이지 마세요. 이름이 "에바"(받침 없음)에서 "골드"·"코인"(받침 있음)으로
...
# default  : 입력받기 전까지 쓸 값 (원본 서버의 이름 그대로예요. 설정을 안 한 기존
#            배포가 지금까지와 똑같이 동작해야 하니까요)
...
# "에바를"처럼 손으로 붙여두면 이름이 "코인"으로 바뀌는 순간 "코인를"이 됩니다.
#
# 짝은 **받침 있을 때가 앞**입니다. josa("코인", "을를") → "을", josa("에바", "을를") → "를"
_JOSA_PAIRS = {
    "을를": ("을", "를"),
    "이가": ("이", "가"),
    "은는": ("은", "는"),
...
def josa(word: str, pair: str) -> str:
    """`word` 뒤에 붙일 조사를 골라줍니다.
        ...
        f"{currency()}{josa(currency(), '을를')}"  → "에바를" / "코인을"
