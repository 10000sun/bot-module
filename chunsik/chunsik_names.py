...
┌─ 쓰는 법 ────────────────────────────────────────────────────────────┐
│   from chunsik_names import currency, bot_name, event_name, josa        │
│                                                                       │
...
from chunsik_config import SETTINGS_FILE
from chunsik_settings import load_settings, save_settings

# ========== 🏷️ 받는 이름들 ==========
# key      : settings.json의 names 아래 저장되는 키
# label    : /초기설정 입력창에 보이는 항목 이름
# default  : 입력받기 전까지 쓸 값.
#            재화·이벤트·서버는 **일반 명사**예요 — 서버마다 반드시 달라지는 값이라,
#            안 정했으면 "1,000 재화"처럼 밋밋한 게 맞습니다. 예전엔 원본 서버의 이름이
#            기본값이라 납품 서버 문구에 남의 서버 이름이 그대로 나갔어요.
#            봇 이름만 예외로 **"김춘식"**입니다. 이 봇의 이름이고, 안 바꾸고 그대로
#            쓰는 서버도 있으니까요. 물론 `/초기설정`에서 얼마든지 바꿀 수 있어요.
...
NAME_FIELDS = {
    ...
    "bot": {
        ...
        "example": "예) 루나, 하루",
        "hint": "봇이 스스로를 부르는 이름",
        # 🏷️ 여기만 일반 명사가 아니라 **진짜 이름**이에요. 다른 셋(재화·이벤트·서버)은
        #    서버마다 반드시 달라지는 값이라 비워두면 어색하지만, 봇 이름은 안 바꾸고
        #    그대로 쓰는 서버가 있어서 기본값이 곧 이 봇의 이름이 됩니다.
    },
    "event": {
        "label": "선착순 이벤트 이름",
        "default": "이벤트",
...
# 재사용해요. (chunsik_settings의 캐시와 같은 방식이라, 파일을 손으로 고쳐도 바로 반영됩니다)
_names_cache = None
