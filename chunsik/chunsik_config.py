...
# .env의 CHUNSIK_DATA_DIR 값을 읽어야 데이터 폴더 위치를 정할 수 있으니까요.
...
#
# .env에서 CHUNSIK_DATA_DIR로 아무 경로나 지정할 수 있어요. 서버 한 대에서
# 여러 봇을 돌릴 때 코드는 한 벌만 두고 데이터 폴더만 나누면 됩니다.
DATA_DIR = os.environ.get("CHUNSIK_DATA_DIR", "").strip() or os.path.join(BASE_DIR, "data")
...
GUILD_CONFIG_PATH = os.environ.get("CHUNSIK_GUILD_CONFIG", "").strip() or os.path.join(BASE_DIR, "guild.json")

# 데이터가 아니라 설정이라서 data/로 이사시키면 안 되는 파일들.
#
# 🐛 [버그 수정] 예전엔 이 집합을 `{basename(GUILD_CONFIG_PATH), "guild.example.json"}`으로만
# 만들었어요. 그런데 CHUNSIK_GUILD_CONFIG로 다른 경로를 지정하면 기본 이름인 guild.json이
...
WIKI_FILE = os.path.join(DATA_DIR, "chunsik_wiki.json")
ATTENDANCE_FILE = os.path.join(DATA_DIR, "chunsik_attendance.json")
STOCKS_FILE = os.path.join(DATA_DIR, "chunsik_stocks.json")
ECONOMY_FILE = os.path.join(DATA_DIR, "chunsik_economy.json")
SHOP_FILE = os.path.join(DATA_DIR, "chunsik_shop.json")
SHOP_TRANSACTIONS_FILE = os.path.join(DATA_DIR, "chunsik_shop_transactions.json")  # 💰 [신규] /정산용 구매·되팔기 거래 내역
CHRONICLE_FILE = os.path.join(DATA_DIR, "chunsik_chronicle.json")  # 📜 [신규] 서버 연대기 (서버에서 일어난 일을 날짜순으로 축적)
...
SETTINGS_FILE = os.path.join(DATA_DIR, "chunsik_settings.json") # 💡 신규 설정 파일
BIRTHDAY_FILE = os.path.join(DATA_DIR, "chunsik_birthday.json")
CHAT_MEMORY_FILE = os.path.join(DATA_DIR, "chunsik_chat_memory.json") # 💬 유저별 마지막 대화 기억 파일
CHAT_LOG_FILE = os.path.join(DATA_DIR, "chunsik_chat_log.json") # 📜 서버별 최근 채팅 로그(맥락 참고용, 개수는 ChunsikGPT.CHAT_LOG_MAXLEN 참고)
CHUNSIK_USER_MEMORY_FILE = os.path.join(DATA_DIR, "chunsik_user_memory.json") # 🧠 [신규] 유저별 장기 기억(사실 목록)
ID_PENDING_FILE = os.path.join(DATA_DIR, "chunsik_id_pending.json") # 🆔 [신규] 아이디 자동등록 중 플랫폼이 불명확해 관리자 확인이 필요한 대기열
PORTFOLIO_HISTORY_FILE = os.path.join(DATA_DIR, "chunsik_portfolio_history.json") # 📈 [신규] 종가게시 시점 유저별 포트폴리오 총 가치 스냅샷
LEDGER_FILE = os.path.join(DATA_DIR, "chunsik_ledger.json") # 🧾 [신규] 재화 입출금 원장 (유저 거래내역 조회 / 지급 되돌리기용)
CHAT_STATS_FILE = os.path.join(DATA_DIR, "chunsik_chat_stats.json") # 💬 [신규] 날짜별 채팅 "개수"만 세는 통계 (내용·작성자 미저장)
HEARTBEAT_FILE = os.path.join(DATA_DIR, "chunsik_heartbeat.txt") # 💓 [신규] 봇이 살아있음을 외부 감시 도구에 알리는 심장박동 파일
BACKUP_DIR = os.path.join(DATA_DIR, "backups") # 💾 매일 자동 백업이 쌓이는 폴더

# 🗑️ [정리] 창고로 간 기능들의 데이터 파일 상수도 같이 걷어냈어요.
# (chunsik_profile.json / chunsik_visit_pass_ask.json / chunsik_camp_*.json / chunsik_exile_data.json,
...
def migrate_legacy_data_files():
    ...
    legacy_heartbeat = os.path.join(BASE_DIR, "chunsik_heartbeat.txt")
...
# 🏷️ 봇 이름이 바뀌면서 데이터 파일 이름의 앞머리도 같이 바뀌었어요.
_RENAMED_DATA_PREFIX = ("mari_", "chunsik_")


def migrate_renamed_data_files():
    """옛 이름(`mari_*.json`)으로 저장돼 있던 데이터를 새 이름으로 옮깁니다.

    🚨 이게 없으면 업데이트하는 순간 **지갑이 0원으로 보입니다.** 코드는 새 이름의
       파일을 찾는데 그런 파일이 없으니, 아래 init_json_files()가 빈 파일을 새로
       만들어주거든요. 데이터가 지워지는 게 아니라 **안 보이게 되는** 종류라 더
       나빠요 — 옛 파일은 그대로 옆에 남아 있는데 아무도 안 읽습니다.

    🔒 새 이름의 파일이 이미 있으면 절대 덮어쓰지 않아요. 위 migrate_legacy와 같은
       원칙입니다 — 새 데이터가 옛 데이터로 되돌아가는 게 제일 나쁜 사고예요.
    """
    old, new = _RENAMED_DATA_PREFIX
    try:
        names = [n for n in os.listdir(DATA_DIR) if n.startswith(old)]
    except Exception:
        return
    ...
    for name in names:
        ...
        dst = os.path.join(DATA_DIR, new + name[len(old):])
        if not os.path.isfile(src):
            continue
        if os.path.exists(dst):
            skipped.append(name)
        ...
        try:
            ...
            print(f"❗ {name} 이름 바꾸기 실패: {type(e).__name__}: {e}")

    if moved:
        print(f"🏷️ 옛 이름의 데이터 파일 {len(moved)}개를 새 이름으로 옮겼어요 "
              f"({old}* → {new}*): {', '.join(moved[:6])}" + (" ..." if len(moved) > 6 else ""))
    if skipped:
        print(f"⚠️ 새 이름의 파일이 이미 있어서 건너뛴 것 ({len(skipped)}개): {', '.join(skipped)}")
        print("   옛 파일은 그대로 뒀어요. 어느 쪽이 최신인지 확인하고 직접 정리해 주세요.")


migrate_legacy_data_files()
migrate_renamed_data_files()   # ⚠️ 반드시 위 함수 뒤에 — 파일이 data/로 모인 다음에 이름을 봐야 해요

# ========== 🏷️ [신규] 서버 고유 ID 분리 (guild.json) ==========
# 예전엔 역할·유저 ID가 코드 곳곳에 숫자로 박혀 있었어요. 이 봇을 다른 서버에 올리려면
# chunsik_config.py / cogs/roster.py / cogs/setting.py / cogs/gpt.py 네 파일을 뒤져가며
...
#
# ⚠️ 여기서 예외를 던지면 안 됩니다. chunsik_config는 main.py가 제일 먼저 import하는
...
CHUNSIK_CALL_ROLE_ID = _role("bot_mention")             # 이 역할을 멘션하면 봇을 부른 것으로 취급
...
#
# 예전엔 chunsik_client가 자기 혼자 resolve_modules()를 불렀어요. 그런데 "지금 뭐가 담겼나"를
...
def active_data_files() -> dict:
    ...
    주식을 안 담고 납품했는데도 `chunsik_stocks.json`이 만들어지고 매일 백업까지 되던 문제를
