"""ChunsikGPT — Gemini 기반 봇 캐릭터 대화, 장기 기억, 채팅 로그/통계."""
...
class ChunsikGPT(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.last_user_message = memory.get("user_to_chunsik", {})   # 유저가 봇한테 한 마지막 말
        self.last_chunsik_message = memory.get("chunsik_to_user", {})   # 봇이 그 유저한테 한 마지막 말
    ...
    def _load_user_memory(self) -> dict:
        return safe_json_load(self.CHUNSIK_USER_MEMORY_FILE, {})

    def _save_user_memory(self, data: dict):
        atomic_json_save(self.CHUNSIK_USER_MEMORY_FILE, data, indent=2)
    ...
    def _load_chat_memory(self) -> dict:
        ...
        default = {"user_to_chunsik": {}, "chunsik_to_user": {}}
        try:
    ...
    def _save_chat_memory(self):
        ...
        atomic_json_save(
            ...
            {"user_to_chunsik": self.last_user_message, "chunsik_to_user": self.last_chunsik_message},
            indent=4
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if is_bot_mentioned or is_role_mentioned:
            ...
            prev_chunsik_msg = self.last_chunsik_message.get(author_key)
            ...
            if prev_user_msg and prev_chunsik_msg:
                conversation.append({"role": "user", "parts": [{"text": prev_user_msg}]})
                conversation.append({"role": "model", "parts": [{"text": prev_chunsik_msg}]})
