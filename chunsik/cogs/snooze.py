"""ChunsikSnooze — 나중에 답장할 메시지를 미뤄뒀다가, 정한 시각에 DM으로 다시 받아보는 기능.

⏰ [이 기능이 뭔가요]
지금은 답장 못 하는데 나중에 꼭 해야 하는 메시지가 있죠. 우클릭 한 번으로 미뤄두면
정한 시각에 봇이 원본 링크와 함께 DM으로 다시 알려줍니다.
...
from chunsik_config import KST, SNOOZE_FILE
...
class SnoozeCustomModal(discord.ui.Modal, title="⏰ 언제 알려드릴까요?"):
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception) -> None:
        ...
        화면엔 아무것도 안 떴습니다. (ChunsikView.on_error와 같은 방식으로 갈라요)
        """
...
class ChunsikSnooze(commands.Cog):
    ...
    def _parse_absolute(cleaned: str, now: dt.datetime) -> Optional[dt.datetime]:
        ...
        🧩 [이동] 실제 구현은 chunsik_utils.parse_datetime_text 에 있어요. 파티 모집이 같은
