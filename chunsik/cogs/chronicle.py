"""ChunsikChronicle — 서버에서 일어난 일을 날짜순으로 쌓아두는 '서버 연대기'.
...
class ChroniclePinModal(discord.ui.Modal, title="📜 연대기에 박제"):
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception) -> None:
        ...
        아무것도 안 뜹니다. 이미 응답했는지 보고 갈라야 해요. (ChunsikView.on_error와 같은 방식)
        """
...
class ChunsikChronicle(commands.Cog):
    """📜 서버 연대기"""
    ...
    def __init__(self, bot):
        ...
        #    때마다 같은 메뉴가 중복 등록됩니다.
        # 🏠 서버 전용 설정은 따로 안 붙입니다. chunsik_client의 tree.interaction_check가
    ...
    def _add_entry(self, *, title: str, detail: str, date: str, tag: str, source: str,
        ...
        그 틈에 들어온 다른 기록이 통째로 되돌아갑니다. (봇 전체가 지키는 규칙이에요)
        """
        data = self._load()
        entry = {
            "id": str(data["next_id"]),
