...
class ChunsikHelp(commands.Cog):
    ...
    def _admin_entries(self):
        ...
        return {
            ...
            "정보 및 역할 관리": ("👥", [
                ...
                ("scrim", "`/내전 정리` - 끝난 내전 기록 지우기 (전적과 순위는 그대로 남아요)"),
                ("scrim", "`/설정 관리자 내전` - 남의 내전도 팀을 짜고 결과를 남길 수 있는 역할 지정"),
    ...
    def _user_entries(self):
        ...
        return {
            ...
            # 🏷️ 제목도 담긴 것만 부릅니다. 둘 다 담으면 "파티 모집 · 내전",
            #    하나만 담으면 그것만. 고정 문자열로 두면 파티를 안 담은 서버에
            #    "파티 모집"이라는 제목만 덩그러니 남아요. (check_help.py가 잡습니다)
            (" · ".join((["파티 모집"] if module_active("party") else [])
                        + (["내전"] if module_active("scrim") else [])) or "모임"): ("🎯", [
                ...
                ("scrim", "`/내전 열기` - 한 팀 인원과 시각을 정해 내전을 열어요 (5를 넣으면 5:5)"),
                ("scrim", "└ 다 모이면 **팀 짜기** 버튼 한 번으로 두 팀이 나뉘어요. 전적을 보고 실력이 한쪽에 몰리지 않게 섞습니다"),
                ("scrim", "└ 경기가 끝나면 버튼으로 결과를 남기세요. 그게 쌓여서 전적과 순위가 됩니다"),
                ("scrim", "`/내전 전적` - 내 승·패와 승률 보기 (다른 사람 것도 볼 수 있어요)"),
                ("scrim", "`/내전 순위` - 승률이 높은 사람 순으로 (3판 이상 한 사람만)"),
