"""ChunsikLevels — 채팅 활동으로 쌓는 레벨. 레벨이 오르면 역할(과 재화)을 줍니다.
...
class ChunsikLevels(commands.Cog):
    ...
    async def _level_up(self, message: discord.Message, level: int, cfg: dict):
        ...
        if role_id:
            ...
            #    (셀프 역할·입장 자동 역할과 같은 검사 — chunsik_utils에 함께 있어요)
        ...
        if amount > 0:
            ...
            economy = self.bot.get_cog("ChunsikEconomy")
    ...
    async def configure(self, interaction: discord.Interaction, 최소: int = None, 최대: int = None,
        ...
        now = self._config()
        note = ""
        if now["coin_per_level"] > 0 and self.bot.get_cog("ChunsikEconomy") is None:
