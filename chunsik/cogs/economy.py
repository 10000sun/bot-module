"""ChunsikEconomy — 재화 지갑, 출석, 송금, 지급/회수, 거래내역, 서버 통계."""
...
# 선물 버튼인데, 아이템은 상점 데이터라 UI·로직이 ChunsikShop 쪽에 있거든요.
#
...
class UndoGiveView(ChunsikView):
    def __init__(self, economy_cog, batches: list):
        super().__init__(timeout=120)
        self.add_item(UndoGiveSelect(economy_cog, batches))


class WalletCleanConfirmView(ChunsikView):
...
class TransferAmountModal(discord.ui.Modal, title="💸 송금"):
    ...
    async def on_error(self, interaction: discord.Interaction, error: Exception) -> None:
        """모달은 ChunsikView와 상속 관계가 아니라서 오류 처리를 따로 붙여둬요."""
...
class ChunsikEconomy(commands.Cog):
    ...
    async def on_ready(self):
        ...
        if not self.midnight_reset_loop.is_running():
            self.midnight_reset_loop.start()
            print("⏰ [ChunsikEconomy] 매일 자정 초기화 루프가 정상적으로 시작됐어요.")

    # 🗑️ [정리] 예전엔 여기 _ensure_files_exist()가 있어서 파일이 없으면 표준 구조를
    # 만들어 넣었어요. 그런데 chunsik_storage.init_json_files()가 봇이 뜰 때 모든 데이터
    ...
    def _load_attendance(self) -> dict:
        ...
        그런데 그 기본값은 **파일이 아예 없을 때만** 쓰입니다. chunsik_storage.init_json_files()가
        ...
        이제 다른 로더들(profile·chunsik_tax·원장)과 똑같이 여기서 빠진 키를 직접 채웁니다.
        """
    ...
    async def _show_all_wallets(self, interaction: discord.Interaction):
        ...
        """
        # 🏠 지금은 chunsik_client._enforce_guild_only가 모든 명령어를 서버 전용으로 막아둬서
