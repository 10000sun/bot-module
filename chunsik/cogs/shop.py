"""ChunsikShop — 채널별 상점 매대, 구매/되팔기, 인벤토리, 월별 정산."""
...
class ShopItemSelect(discord.ui.Select):
    ...
    def __init__(self, cog, channel_id: int):
        ...
        super().__init__(
            ...
            max_values=1,
            options=options,
            custom_id=f"chunsik_shop:item_select:{channel_id}"
        )
...
class ShopActionButtons(ChunsikView):
...
class GiftPanelView(ChunsikView):
...
class WalletGiftView(ChunsikView):
...
class ChunsikShop(commands.Cog):
    """서버 공식 상점 및 인벤토리 시스템"""
    ...
    def _register_persistent_views(self):
        ...
        🧩 [이동] 예전엔 이 코드가 chunsik_client.setup_hook 안에 있었어요. 그래서 클라이언트
    ...
    def build_wallet_gift_view(self, owner_id: int) -> "WalletGiftView":
        ...
        🧩 지갑(ChunsikEconomy)이 이 버튼을 붙이는데, 선물은 상점 데이터(인벤토리)를 만지는
    ...
    def _load_economy_file(self) -> dict:
        """ChunsikEconomy 지갑 시스템에서 실시간으로 정밀 경제 데이터를 가져옵니다."""
        # 1. 로드된 ChunsikEconomy Cog가 있다면 해당 인스턴스의 로드 함수를 최우선 공유
    ...
    def _save_economy_file(self, economy_dict: dict):
        ...
        # 1. ChunsikEconomy Cog의 저장 함수를 호출하여 실시간 동기화
        economy_cog = self.bot.get_cog("ChunsikEconomy")
    ...
    def _set_balance(self, user_id: int, amount: int):
        ...
        # ChunsikEconomy 규격과 완전히 동일하게 단일 정수 값으로 저장 포맷 일치화
    ...
    def _record_transaction(self, channel_id: int, item_name: str, tx_type: str, amount: int):
        ...
        빠지는 건 아쉽지만 거래 자체를 깨뜨리는 것보다는 나아요. (chunsik_state.record_ledger와 같은 판단)
        """
        try:
