"""ChunsikWelcome — 입장 자동화. 새로 들어온 사람에게 역할을 달아주고 인사합니다.
...
🔐 여기서도 권한 상승을 막습니다. 자동 역할과 동의 역할은 **본인 확인 없이 붙는**
   ...
   셀프 역할과 같은 검사를 씁니다. (chunsik_utils.role_reject_reason — 두 코그가 공유해요)
...
from chunsik_state import load_welcome, save_welcome
from chunsik_utils import (EMBED_DESC_LIMIT, EMBED_FIELD_LIMIT,
                        EMBED_TITLE_LIMIT, ChunsikView, clip,
...
class RuleAgreeView(ChunsikView):
    """규칙 동의 버튼. timeout=None이라 봇을 재시작해도 계속 눌려요."""

    def __init__(self, cog: "ChunsikWelcome", label: str = "규칙에 동의합니다"):
...
class ChunsikWelcome(commands.Cog):
