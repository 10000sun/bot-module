"""ChunsikStock — 주식 매매, 종가 게시, 주가 그래프, 개폐장 관리."""
...
from chunsik_config import CHART_BG, CHART_DOWN, CHART_GRID, CHART_INK, CHART_INK_SOFT, CHART_UP, ECONOMY_FILE, KST, PORTFOLIO_HISTORY_FILE, STOCKS_FILE, _CHART_EMOJI_RE
...
class ClosingPriceView(ChunsikView):
            ...
            async with self.bot.economy_lock:
                ...
                if not found_stock or "price" not in stocks_dict[found_stock]:
                    ...
                    if user_bal < total_cost:
                        ...
                        # 읽는 방법을 공용 함수(chunsik_utils)로 통일했습니다.
