...
from chunsik_settings import send_log_embed
...
def portfolio_value(stock_data: dict, user_id) -> int:
    ...
    stock_data는 ChunsikStock._load_stocks()가 돌려주는 전체 구조 그대로 넘기면 돼요.
...
def describe_user_error(error: BaseException) -> str:
    ...
    슬래시 명령어(chunsik_client.on_app_command_error)와 버튼/드롭다운(ChunsikView.on_error)이
