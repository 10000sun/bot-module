...
주문제작마다 담는 기능이 달라서, 예전처럼 chunsik_client.py에 import 17줄과 add_cog 17줄을
손으로 적어두면 기능 하나 빼고 넣을 때마다 클라이언트 본체를 고쳐야 했어요. 이제 여기
목록만 보고 로드하며, 무엇을 켤지는 guild.json의 modules 항목이 정합니다.

다른 봇 모듈을 import하지 않습니다. (chunsik_config보다도 아래, 순수 자료 계층이에요)
"""
...
class ModuleSpec:
    ...
    예전엔 이 목록들이 setting.py·help.py·diagnostics.py·chunsik_settings.py에 각각
    손으로 적혀 있어서, 기능을 뺄 때마다 네 군데를 같이 고쳐야 했고 늘 하나씩 빠뜨렸어요.

    ⚠️ 여기에 적는 건 전부 **문자열 키**입니다. 이 파일은 다른 봇 모듈을 import하지 않는
       최하위 자료 계층이라, 실제 상수나 경로를 직접 참조하면 안 돼요.
       (data_files는 chunsik_config의 전역 '이름'이고, 경로로 바꾸는 건 chunsik_config가 합니다)
    """
    ...
    data_files: tuple = ()  # chunsik_config의 *_FILE 전역 '이름' (경로가 아니라 이름이에요)
    note: str = ""


# ⚠️ 여기 적힌 **순서가 곧 로드 순서**입니다. 원본 봇의 add_cog 순서를 그대로 옮겨왔어요.
# 로드 순서가 동작을 바꾸는 부분은 지금 없지만(코그끼리는 전부 런타임에 get_cog로 찾습니다),
# 명령어가 디스코드에 등록되는 순서가 달라지므로 이유 없이 흔들지 않는 게 좋아요.
MODULE_SPECS = (
    ModuleSpec(
        "setting", "설정·권한·기능제어", "cogs.setting", "ChunsikSetting", core=True,
        channels=("role_log",),
    ...
    ModuleSpec(
        "id", "게임 아이디 등록부", "cogs.ids", "ChunsikIds",
        ...
        note="예전 파일 이름은 cogs/core.py였어요. '핵심'처럼 보이는데 실제 내용은 게임 아이디 "
             "등록/조회라 헷갈려서 cogs/ids.py로 바꿨습니다. (클래스도 ChunsikCore → ChunsikIds)",
    ),
    ModuleSpec(
        "economy", "지갑·송금·출석", "cogs.economy", "ChunsikEconomy",
    ...
    ModuleSpec(
        "stock", "모의 주식", "cogs.stock", "ChunsikStock", requires=("economy",),
    ...
    ModuleSpec(
        "shop", "상점·인벤토리·선물", "cogs.shop", "ChunsikShop", requires=("economy",),
        channels=("shop_log", "shop_board"),
        roles=("shop_admin",),
        features=("shop",),
        data_files=("SHOP_FILE", "SHOP_TRANSACTIONS_FILE"),
    ),
    ModuleSpec("help", "도움말", "cogs.help", "ChunsikHelp", core=True),
    ModuleSpec(
        "wizard", "설치 마법사", "cogs.wizard", "ChunsikWizard", core=True,
    ...
    ModuleSpec(
        "birthday", "생일 등록·축하", "cogs.birthday", "ChunsikBirthday",
        channels=("birthday_announce", "birthday_log"),
        features=("birthday",),
        data_files=("BIRTHDAY_FILE",),
    ),
    ModuleSpec(
        "diagnostics", "진단·데이터 점검", "cogs.diagnostics", "ChunsikTest", core=True,
        roles=("test_admin",),
        note="켜져 있는 모듈만 골라서 점검해요. 문제가 생겼을 때 원인을 찾는 유일한 창구라 항상 포함합니다.",
    ),
    ModuleSpec(
        "backup", "자동 백업", "cogs.backup", "ChunsikBackup", core=True,
        note="매일 데이터 폴더를 통째로 백업해요. 어떤 기능을 담든 데이터는 지켜야 하므로 항상 포함합니다.",
    ),
    ModuleSpec(
        "chronicle", "서버 연대기", "cogs.chronicle", "ChunsikChronicle",
        roles=("chronicle_admin",),
        data_files=("CHRONICLE_FILE",),
    ),
    ModuleSpec(
        "snooze", "나중에 답장", "cogs.snooze", "ChunsikSnooze",
        features=("snooze",),
    ...
    ModuleSpec(
        "welcome", "입장 자동화", "cogs.welcome", "ChunsikWelcome",
        channels=("welcome", "member_log"),
        roles=("welcome_admin",),
        features=("welcome",),
        data_files=("WELCOME_FILE",),
        note="SERVER MEMBERS INTENT가 있어야 입장 자체가 봇에게 들어와요. (없으면 봇이 기동조차 못 하니 "
             "실제로는 늘 켜져 있습니다) 역할 안전 검사는 셀프 역할과 함께 chunsik_utils에 있어요.",
    ),
    ModuleSpec(
        "selfrole", "셀프 역할", "cogs.selfrole", "ChunsikSelfRole",
